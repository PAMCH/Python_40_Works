...
import threading
import os
import schedule # 일정시간마다 함수을 동작시키는 모듈
...
schedule.every(30).minutes.do(send_message()) # 30분마다 함수 실행
# schedule.every().monday.at("09:10").do(send_message()) # 매주 월요일 9시 10분마다 실행
# schedule.every().day.at("10:30").do(send_message()) # 매일 10시 30분마다 실행